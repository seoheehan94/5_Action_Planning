...
% ---------- Configuration ----------
expName     = 'WM_GOSTOP_EXP6'; % experiment name for data files
expData     = 'result_WM_GOSTOP_mismatchaction'; % experiment data folder name
...
for blk = 1:block
    ...
    % word  1 = no-action(match), 2 = action(mismatch)
...
for blk = 1:block
    [F1, F2, F3] = BalanceFactors(1,1, [1,2], [1, 2, 3, 4, 5, 6], [1, 0]);
    % action  1 = no-action(match), 2 = action(mismatch)
    % prime color 1=red, 2=yellow, 3=green, 4=blue, 5=purple, 6=gray
    % validity  1 = valid, 0 = invalid
...
for P = 1:pracN
    ...
    wordP       = trialMat(P,1); % 1 = no-action(match), 2 = action(mismatch)
    ...
    if wordP == 1 % no-action (match)
        DrawTextUni(w, wordCueList{priColP});
    elseif wordP == 2 % action (mismatch)
        anoWord = [1,2,3,4,5,6];
...
for T = 1:trialN
    ...
    wordT       = trialMat(T,1); % 1 = no-action(match), 2 = action(mismatch)
    ...
    if wordT == 1 % no-action (match)
        DrawTextUni(w, wordCueList{priColT});
    elseif wordT == 2 % action (mismatch)
        anoWord = [1,2,3,4,5,6];
