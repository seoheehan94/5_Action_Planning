...
% Version       : 2018.0327
% Note          : action planning finding T/L 
...
% ---------- Configuration ----------
expName     = 'WM_GOSTOP_TL'; % experiment name for data files
expData     = 'result_WM_GOSTOP_TL'; % experiment data folder name
...
[w, rect] = Screen('OpenWindow', 0, bgColor); % open main screen
...
%     Screen('DrawLine',w, txtColor, cx, cy-10, cx, cy+10, 3);
    ...
    % Correct
    if letterP == 1
        memCorrP = letterP == memRespP;
    elseif letterP == 2
        memCorrP = (letterP+97) == memRespP;
    end
    ...
    % write data
    PRACDATA(P,:) = [P, letterP, priColP, realAnoCol, cueRespP, cueCorrP, cueRTP, validP, targLoc, anoLoc, targOri, srcRespP, srcCorrP, srcRTP, memRespP, memCorrP, memRTP];
    save([fileName,'.mat']);
...
%     Screen('DrawLine',w, txtColor, cx, cy-10, cx, cy+10, 3);
    ...
    % Correct
    if letterT == 1
        memCorrT = letterT == memRespT;
    elseif letterT == 2
        memCorrT = (letterT+97) == memRespT;
    end
    ...
    % write data
    RAWDATA(T,:) = [T, letterT, priColT, realAnoCol, cueRespT, cueCorrT, cueRTT, validT, targLoc, anoLoc, targOri, srcRespT, srcCorrT, srcRTT, memRespT, memCorrT, memRTT];
    save([fileName,'.mat']);
...
RestrictKeysForKbCheck([WMKeys{1} WMKeys{2} WMKeys{3} WMKeys{4}]); % left, right 만 허용
    ...
    WMDATA(1,:) = [WMletter, WMvalid, targOri, WMsrcResp, WMsrcCorr, WMsrcRT, WMCol, realAnoCol, WMResp, WMCorr, cfWMResp];
    save([fileName,'.mat']);
